...
The multinomial logit assumes that the relative odds between any two alternatives are independent of the presence of other alternatives (IIA). Table~\ref{tab:app_iia_formal} reports Hausman-McFadden formal tests for each alternative. IIA is rejected for the Neither and S+A categories ($\chi^2 = 1017.6$ and $\chi^2 = 1257.5$ respectively, both $p < 0.001$), confirming that these two alternatives represent genuinely distinctive preference profiles rather than arbitrary category distinctions. IIA is not rejected for Sanctions-only ($\chi^2 = 19.4$, $p = 0.989$) or Action-only ($\chi^2 = -3.5$, $p = 1.000$), indicating that these two categories are more interchangeable in the choice set. The pattern is theoretically coherent: S+A and Neither occupy distinct attitudinal regions, while Sanctions-only and Action-only behave more like substitutes. The negative chi-square value for Action-only arises when the restricted-model variance--covariance matrix is not positive definite, a known property of the Hausman-McFadden procedure that indicates no evidence against IIA for that alternative.
