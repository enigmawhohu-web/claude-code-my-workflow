...
\centering
\small
\caption{Design-Based Sensitivity Check: MNL S+A Blame Contrasts}
\label{tab:app_design_mnl}
\input{tables/app_tab_design_mnl}
\par\vspace{0.5ex}
{\footnotesize\textit{Notes.} Design-based standard errors computed via \texttt{svyglm()} binary contrasts (S+A vs.\ all other categories). The binary-contrast approach approximates the design-based equivalent of the full MNL. Point estimates are consistent with the main multinomial specification.}
\end{table}
...
\input{tables/app_tab_mi_phase5}
\par\vspace{0.5ex}
{\footnotesize\textit{Notes.} Pooled MI estimates ($m = 10$ imputations, PMM) for the stacked-OLS probe contrasts. The target probe contrast ($\Delta_d$) and composite boundary mean are stable under full-sample imputation.}
\end{table}
...
\input{tables/app_tab_mi_diagnostics}
\par\vspace{0.5ex}
{\footnotesize\textit{Notes.} Rubin's rules variance decomposition. $V_W$ = within-imputation variance; $V_B$ = between-imputation variance; FMI = fraction of missing information. Low FMI ($\approx 0.001$) confirms that missingness contributes negligible uncertainty to the pooled estimates.}
\end{table}

% TODO: Measurement Appendix (M-2) — add verbatim question wording, response options, and
% variable construction details for all key measures (warmth, blame, posture, tool config,
% 8-probe battery). Requires codebook PDFs; out of scope for current revision pass.
